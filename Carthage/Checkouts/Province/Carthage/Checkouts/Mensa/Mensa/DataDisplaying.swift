...
public extension DataDisplaying where Self: UIViewController, DataSourceType.Item == Item {
    ...
    func register<T, ViewController: UIViewController>(_ itemType: T.Type, conformingTypes: [Any.Type] = [], with viewControllerType: ViewController.Type) where ViewController: ItemDisplaying, T == ViewController.Item {
        dataMediator?.register(itemType, conformingTypes: conformingTypes, with: viewControllerType)
    }
